...
**Note:** `tui_e2e_runner.sh` exists but uses FLAT structure. You MUST update it to use **per-feature subdirectories**:

```bash
# ⚠️ AGENT TASK: Update tui_e2e_runner.sh to create per-feature directories

# Current implementation (FLAT - needs changing):
SNAPSHOTS_DIR="$SCRIPT_DIR/tui/snapshots"
# Output: snapshots/terminal_layout_initial.png

# Required implementation (PER-FEATURE):
FEATURE_DIR="$SCRIPT_DIR/tui/snapshots/$FEATURE_NAME"
mkdir -p "$FEATURE_DIR"
# Output: snapshots/01_terminal_layout/initial.png, snapshots/01_terminal_layout/scroll_top.png, etc.
```
...
# 4. VERIFY snapshot directory created (PER-FEATURE structure)
ls -la tests/visual/tui/snapshots/XX_feature_name/
# Expected: initial.png, final.png, and scenario-specific snapshots

# 5. VERIFY recording directory created  
ls -la tests/visual/tui/recordings/XX_feature_name/
# Expected: demo.gif and scenario-specific recordings
...
**⚠️ NEW TUI uses PER-FEATURE SUBDIRECTORIES (different from chat/ which is flat).**

This is intentional - TUI has 15 features with many scenarios, so per-feature organization is needed.

```
tests/visual/tui/
├── snapshots/                              # PNG baselines (COMMITTED to git)
...
│   ├── 03_message_display/                 # Feature 03 directory
│   │   ├── system_message.png
│   │   ├── user_message.png
│   │   ├── agent_message.png
│   │   ├── tool_message.png
│   │   └── thinking_block.png
│   ├── 04_input_area/
│   ├── 05_modals_provider_picker/
│   ├── 06_modals_model_picker/
│   ├── 07_modals_file_picker/
│   ├── 08_modals_theme_picker/
│   ├── 09_modals_help/
│   ├── 10_questions_multiple_choice/
│   ├── 11_questions_single_choice/
│   ├── 12_questions_free_text/
│   ├── 13_sidebar/
│   ├── 14_theming/
│   ├── 15_keyboard_shortcuts/
│   └── complete_feature_test/              # 🎯 Complete demo
│       └── final_state.png
│
...
│
└── logs/                                   # E2E run logs (GITIGNORED)
    └── *.log
```

**Why Per-Feature Directories (different from chat/):**
- TUI has **15 features** with **245+ scenarios** (vs chat's 7 features)
- Each feature may have **multiple snapshots** for different states
- Easier to navigate, review, and maintain
- Clear mapping: feature file → snapshot directory → recording directory

**Agent Task:** Update `tui_e2e_runner.sh` to use per-feature directories:
```bash
# Current (flat - needs updating):
SNAPSHOTS_DIR="$SCRIPT_DIR/tui/snapshots"

# Should be (per-feature):
SNAPSHOTS_DIR="$SCRIPT_DIR/tui/snapshots/$FEATURE_NAME"
```
...
| No `snapshots/XX_feature/` directory | E2E not capturing | Fix tui_e2e_runner.sh to create per-feature dirs |
| No `recordings/XX_feature/` directory | Recording failed | Check asciinema/agg setup |
| Directory exists but EMPTY | Scenario not running | Check expect script for this feature |
...
# 2. Verify per-feature directories created
ls tests/visual/tui/current/02_status_bar/
# Expected: mode_build.png, mode_plan.png, mode_ask.png, etc.

ls tests/visual/tui/recordings/02_status_bar/
# Expected: mode_switching.gif

# 3. If directories are EMPTY or MISSING → E2E runner needs fixing!

# 4. Update baseline
./tests/visual/tui_e2e_runner.sh --update-baseline

# 5. Verify baseline updated
ls tests/visual/tui/snapshots/02_status_bar/
# Expected: All scenario PNGs copied from current/
```
...
### Complete Recording Structure (Per-Feature)

```
tests/visual/tui/
├── snapshots/                        # Per-feature snapshot directories
│   ├── 01_terminal_layout/
│   │   ├── initial.png
│   │   ├── final.png
│   │   └── scroll_*.png
...
│   ├── 15_keyboard_shortcuts/
│   │   └── shortcut_*.png
│   └── complete_feature_test/        # 🎯 FINAL COMPLETE
│       └── final_state.png
│
├── recordings/                       # Per-feature recording directories
│   ├── 01_terminal_layout/
│   │   └── demo.gif
│   ├── 02_status_bar/
│   │   └── mode_switching.gif
...
│       ├── complete_feature_test.cast   # Raw asciinema
│       └── complete_feature_test.gif    # Animated demo (~2-3 min)
```
...
1. ✅ All 270 BDD scenarios pass against REAL `tark tui` (not mocks!)
2. ✅ All 15 feature directories exist in `snapshots/`:
   ```
   snapshots/01_terminal_layout/  ← Has PNGs
   snapshots/02_status_bar/       ← Has PNGs
   ...
   snapshots/15_keyboard_shortcuts/ ← Has PNGs
   ```
3. ✅ All 15 feature directories exist in `recordings/`:
   ```
   recordings/01_terminal_layout/  ← Has GIFs
   recordings/02_status_bar/       ← Has GIFs
   ...
   recordings/15_keyboard_shortcuts/ ← Has GIFs
   ```
4. ✅ `complete_feature_test/` exists with full demo recording
5. ✅ Visual regression passes (RMSE < 0.1)
6. ✅ All artifacts committed to git
7. ✅ `tui_e2e_runner.sh` updated to use per-feature directories
