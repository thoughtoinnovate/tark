...
- 🔨 `tests/cucumber_tui.rs` - Cucumber test harness for BDD tests
- 🔨 `tests/visual/tui_e2e_runner.sh` - E2E runner for TUI (adapt from e2e_runner.sh)
- 🔨 `src/main.rs` - Add `Tui` subcommand (CLI wiring)
- 🔨 `src/transport/cli.rs` - Add `run_tui()` function
...
Step definitions link Gherkin steps to Rust code:
...
async fn see_header(world: &mut TuiWorld) {
    assert!(world.app.as_ref().unwrap().has_header());
}
```
...
async fn status_bar_shows(world: &mut TuiWorld, expected: String) {
    ...
    let status = app.get_status_bar_text();
    assert!(status.contains(&expected), 
        "Expected status bar to contain '{}', got '{}'", expected, status);
}
```
