...
#[test]
fn test_plan_mode_excludes_shell() {
    let tmp = TempDir::new().unwrap();
    let registry = ToolRegistry::for_mode(tmp.path().to_path_buf(), AgentMode::Plan, true);
    let tool_names = get_tool_names(&registry);
    ...
    assert!(
        ...
        "Plan mode should NOT have shell tool. Available tools: {:?}",
        tool_names
    );
}
...
#[tokio::test]
async fn test_plan_mode_rejects_shell_execution() {
    let tmp = TempDir::new().unwrap();
    let registry = ToolRegistry::for_mode(tmp.path().to_path_buf(), AgentMode::Plan, true);
    ...
    assert!(!result.success, "shell should fail in Plan mode");
    assert!(
        result.output.contains("Unknown tool"),
        "Error should indicate unknown tool. Got: {}",
        result.output
...
#[test]
fn test_review_mode_has_write_tools() {
    let tmp = TempDir::new().unwrap();
    let registry = ToolRegistry::for_mode(tmp.path().to_path_buf(), AgentMode::Review, true);
    let tool_names = get_tool_names(&registry);

    // Review mode should have same tools as Build mode
    assert!(
        tool_names.contains(&"write_file".to_string()),
        "Review mode should have write_file tool"
    );
    assert!(
        tool_names.contains(&"patch_file".to_string()),
        "Review mode should have patch_file tool"
    );
    assert!(
        tool_names.contains(&"delete_file".to_string()),
        "Review mode should have delete_file tool"
    );
}
...
fn test_plan_mode_has_fewer_tools_than_build_mode() {
    ...
    // Note: Plan has propose_change which Build doesn't, so it's not strictly fewer,
    // but it should not have the dangerous write tools
    let write_tools = vec!["write_file", "patch_file", "delete_file", "shell"];
