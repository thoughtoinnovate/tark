...
fn test_ask_mode_no_approval_required() -> Result<()> {
    ...
    assert!(!decision.needs_approval, "Ask mode should not require approval");
    Ok(())
}
...
fn test_plan_mode_no_approval_required() -> Result<()> {
    ...
    assert!(!decision.needs_approval, "Plan mode should not require approval");
    Ok(())
}
...
fn test_build_balanced_read_in_workdir_auto_approved() -> Result<()> {
    ...
    assert!(!decision.needs_approval, "Read in workdir should be auto-approved");
    assert_eq!(decision.classification.operation, tark_cli::policy::Operation::Read);
...
fn test_build_balanced_read_outside_workdir_auto_approved() -> Result<()> {
    ...
    assert!(!decision.needs_approval, "Read outside workdir should be auto-approved in balanced");
    assert_eq!(decision.classification.operation, tark_cli::policy::Operation::Read);
    assert!(!decision.classification.in_workdir);
    Ok(())
}
...
fn test_build_careful_read_outside_workdir_needs_approval() -> Result<()> {
    ...
    assert!(decision.needs_approval, "Read outside workdir should need approval in careful");
...
fn test_build_balanced_write_in_workdir_auto_approved() -> Result<()> {
    ...
    assert!(!decision.needs_approval, "Write in workdir should be auto-approved in balanced");
    assert_eq!(decision.classification.operation, tark_cli::policy::Operation::Write);
    assert!(decision.classification.in_workdir);
    Ok(())
}
...
fn test_build_balanced_write_outside_workdir_needs_approval() -> Result<()> {
    ...
    assert!(decision.needs_approval, "Write outside workdir should need approval");
    assert!(decision.allow_save_pattern, "Should allow saving pattern");
    assert!(!decision.classification.in_workdir);
    Ok(())
}
...
fn test_build_careful_write_in_workdir_needs_approval() -> Result<()> {
    ...
    assert!(decision.needs_approval, "Write in workdir should need approval in careful");
    assert!(decision.allow_save_pattern, "Should allow saving pattern");
    Ok(())
}
...
fn test_build_balanced_rm_in_workdir_auto_approved() -> Result<()> {
    ...
    assert!(!decision.needs_approval, "Rm in workdir should be auto-approved in balanced");
    assert_eq!(decision.classification.operation, tark_cli::policy::Operation::Delete);
    assert!(decision.classification.in_workdir);
    Ok(())
}
...
fn test_build_balanced_rm_outside_workdir_needs_approval() -> Result<()> {
    ...
    assert!(decision.needs_approval, "Rm outside workdir should need approval");
    assert!(decision.allow_save_pattern, "Should allow saving pattern in balanced");
    assert!(!decision.classification.in_workdir);
    Ok(())
}
...
fn test_build_careful_rm_outside_workdir_always_prompts() -> Result<()> {
    ...
    assert!(decision.needs_approval, "Rm outside workdir should ALWAYS need approval in careful");
    assert!(!decision.allow_save_pattern, "Should NOT allow saving pattern (ALWAYS mode)");
    assert!(!decision.classification.in_workdir);
    Ok(())
}
...
fn test_build_manual_rm_outside_workdir_always_prompts() -> Result<()> {
    ...
    assert!(decision.needs_approval, "Rm outside workdir should ALWAYS need approval in manual");
    assert!(!decision.allow_save_pattern, "Should NOT allow saving pattern (ALWAYS mode)");
    Ok(())
}
...
fn test_path_detection_relative() -> Result<()> {
    ...
    assert!(decision.classification.in_workdir, "Relative path should be in_workdir");
    Ok(())
}
...
fn test_path_detection_absolute() -> Result<()> {
    ...
    assert!(!decision.classification.in_workdir, "Absolute path /tmp should be outside workdir");
    Ok(())
}
