...
function M.show_summary()
    ...
    -- Use pcall to catch plenary.curl errors (e.g., connection refused)
    local curl_ok, err = pcall(function()
        curl.get(url, {
    ...
    if not curl_ok then
        vim.notify('Failed to connect to server: ' .. tostring(err), vim.log.levels.ERROR)
    end
end
