...
use crate::llm::{Message, MessageContent, ContentPart, Role, ToolCall};
...
impl ConversationContext {
    ...
    fn trim(&mut self) {
        ...
        self.messages.extend(non_system.into_iter().skip(skip_count));
    }
    ...
    fn trim_by_tokens(&mut self) {
        while self.estimate_total_tokens() > self.max_context_tokens && self.messages.len() > 2 {
            ...
            if let Some(idx) = self.messages.iter().position(|m| m.role != Role::System) {
                ...
                tracing::debug!("Trimmed message at index {} to stay within token limit", idx);
            } else {
                break;
            }
        }
    ...
    pub fn compact_with_summary(&mut self, summary: &str, keep_recent: usize) {
        ...
        let recent_messages: Vec<Message> = non_system.into_iter()
            .skip(recent_start)
            .collect();
        ...
        tracing::info!("Compacted context: {} messages remaining, ~{} tokens",
            self.messages.len(), self.estimate_total_tokens());
    }
