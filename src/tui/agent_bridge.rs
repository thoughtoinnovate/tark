...
pub enum AgentMode {
    ...
    Ask,
    /// Plan mode: read + propose changes
    Plan,
...
impl From<AgentMode> for ToolAgentMode {
    fn from(mode: AgentMode) -> Self {
        match mode {
            ...
            AgentMode::Ask => ToolAgentMode::Ask,
        }
    }
}
...
impl From<ToolAgentMode> for AgentMode {
    fn from(mode: ToolAgentMode) -> Self {
        match mode {
            ...
            ToolAgentMode::Ask => AgentMode::Ask,
        }
    }
}
...
impl AgentBridge {
    ...
    pub fn with_provider_and_interaction(
        ...
        // Get approval mode from session
        let trust_level = current_session.approval_mode.parse().unwrap_or_default();
    ...
    /// Set the trust level for risky operations
    ///
    /// This updates both the local state and the ChatAgent's tool registry
    /// so that the approval gate will use the new level when checking tool execution.
    pub fn set_trust_level(&mut self, level: crate::tools::TrustLevel) {
        self.trust_level = level;
        // Save to session for persistence (using approval_mode field name for backward compat)
        self.current_session.approval_mode = level.as_storage_str().to_string();
        let _ = self.storage.save_session(&self.current_session);
        tracing::info!("Trust level set to: {} {}", level.icon(), level.label());
    }

    /// Set the trust level asynchronously (updates the tool registry)
    pub async fn set_trust_level_async(&mut self, level: crate::tools::TrustLevel) {
        self.trust_level = level;
        // Save to session for persistence (using approval_mode field name for backward compat)
        self.current_session.approval_mode = level.as_storage_str().to_string();
        let _ = self.storage.save_session(&self.current_session);
        self.agent.set_trust_level(level).await;
        tracing::info!(
            "Trust level set to: {} {} (synced to agent)",
            level.icon(),
            level.label()
        );
    }
...
impl AgentBridge {
    ...
    pub fn switch_session(&mut self, session_id: &str) -> Result<()> {
        ...
        // Update trust level from session
        self.trust_level = session.approval_mode.parse().unwrap_or_default();
...
mod tests {
    ...
    fn test_agent_mode_conversion() {
        ...
        assert_eq!(AgentMode::from("ask"), AgentMode::Ask);
    ...
    fn test_agent_mode_to_tool_mode() {
        ...
        let tool_mode: ToolAgentMode = AgentMode::Ask.into();
        assert_eq!(tool_mode, ToolAgentMode::Ask);
    }
    ...
    fn test_agent_mode_next_cycle() {
        ...
        assert_eq!(AgentMode::Plan.next(), AgentMode::Ask);
        assert_eq!(AgentMode::Ask.next(), AgentMode::Build);
    }

    #[test]
    fn test_agent_mode_prev_cycle() {
        // Test reverse cycling: Build → Review → Plan → Build
        assert_eq!(AgentMode::Build.prev(), AgentMode::Ask);
        assert_eq!(AgentMode::Ask.prev(), AgentMode::Plan);
    ...
    fn test_agent_mode_next_prev_inverse() {
        ...
        for mode in [AgentMode::Build, AgentMode::Plan, AgentMode::Ask] {
...
mod property_tests {
    ...
    proptest! {
        ...
        fn prop_mode_cycling_forward(
            ...
            let modes = [AgentMode::Build, AgentMode::Plan, AgentMode::Ask];
