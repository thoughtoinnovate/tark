...
pub enum CommandResult {
    ...
    Message(String),
    /// Change trust level
    ChangeTrustLevel(TrustLevelChange),
    /// Show trust level selector popup
    ShowTrustSelector,
    /// Show plan picker popup (accessible from all modes)
    PlansPicker,
...
/// Trust level change options
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum TrustLevelChange {
    /// Auto-run reads & writes, prompt for shell/delete (default)
    Balanced,
    /// Auto-run reads only, prompt for writes+
    Careful,
...
impl CommandHandler {
    ...
    fn register_all(&mut self) {
        ...
        self.register(Command {
            name: "ask".to_string(),
            description: "Switch to ask mode (read-only)".to_string(),
            usage: "/ask".to_string(),
            category: CommandCategory::Mode,
            requires_args: false,
            requires_editor: false,
        ...
        self.register(Command {
            ...
            description: "Set approval mode (risky/all/paranoid) or cycle".to_string(),
            usage: "/approval [risky|all|paranoid]".to_string(),
            category: CommandCategory::Mode,
            requires_args: false,
            requires_editor: false,
        });
        ...
        // Plan commands (simplified to single /plans command)
        self.register(Command {
            name: "plans".to_string(),
            description: "View and manage execution plans".to_string(),
            usage: "/plans".to_string(),
    ...
    pub fn execute_with_editor_state(&self, input: &str, editor_connected: bool) -> CommandResult {
        ...
        match resolved_name {
            ...
            "ask" => CommandResult::ChangeMode(AgentModeChange::Ask),

            // Approval mode command - opens popup selector, or sets specific mode
            "approval" => {
                if args.is_empty() {
                    // Open the approval mode selector popup
                    CommandResult::ShowTrustSelector
                } else {
                    let level = args.to_lowercase();
                    match level.as_str() {
                        // New names
                        "balanced" => CommandResult::ChangeTrustLevel(TrustLevelChange::Balanced),
                        "careful" => CommandResult::ChangeTrustLevel(TrustLevelChange::Careful),
                        "manual" => CommandResult::ChangeTrustLevel(TrustLevelChange::Manual),
                        // Backward compatibility with old names
                        "risky" | "ask-risky" => {
                            CommandResult::ChangeTrustLevel(TrustLevelChange::Balanced)
                        }
                        "reads" | "only-reads" => {
                            CommandResult::ChangeTrustLevel(TrustLevelChange::Careful)
                        }
                        "zero" | "zero-trust" => {
                            CommandResult::ChangeTrustLevel(TrustLevelChange::Manual)
                        }
                        _ => CommandResult::Error(format!(
                            "Unknown trust level: '{}'. Use: balanced, careful, or manual",
                            level
                        )),
                    }
                }
            ...
            // Plan commands (simplified to /plans)
            "plans" => CommandResult::PlansPicker,
...
mod tests {
    ...
    fn test_execute_mode_commands() {
        ...
        assert_eq!(
            handler.execute("/ask"),
            CommandResult::ChangeMode(AgentModeChange::Ask)
        );
    }
    ...
    fn test_get_completions_partial() {
        ...
        assert!(completions.contains(&"/plans".to_string()));
    }
    ...
    fn test_command_categories() {
        ...
        let plans = handler.get_command("plans").unwrap();
        assert_eq!(plans.category, CommandCategory::Plan);
