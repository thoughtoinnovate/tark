...
| Backend → client | `agentInfo.{name,version}` | `tark` + `CARGO_PKG_VERSION` (currently `0.12.6`). Clients must compare major/minor against their supported range and surface a remediation path on mismatch. |
